...
for ((i=0; i<1; i++))
	do  for j in ${array[@]}
		...
		python3 multi_agents_threaded.py --simTime=600 \
			...
			--adjacency_matrix_path=scratch/my_network/examples/geant/adjacency_matrix.txt \
			--node_coordinates_path=scratch/my_network/examples/geant/node_coordinates.txt \
			--load_factor=$j &		
		counter=$((counter+1)) &
        sleep 10
	done
