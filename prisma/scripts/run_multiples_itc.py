...
dqn_models = ["sp", "opt"]
dqn_models = [""]
experiment_name = "ITC_NN_size_variations_experiment_rcpo_final_01_june_2023_working_rcpo_inband"
                                                ...
                                                for sync_step in sync_steps:
                                                    for d_m_max_time in d_t_max_times:
                                                        if d_m_max_time != 1 and signaling_type == "NN":
                                                            continue
                                                        if sync_step != 3 and signaling_type == "digital_twin":
                                                            continue
                                                        
                                                        session_name = f"sync_{sync_step}_mat_{traff_mat}_dqn_{dqn_model}_{signaling_type}_size_{nn_sizes[idx]}_{topology_name}_tr_{training_load}_sim_{train_duration}_lr_{lr}_bs_{bs}_outb_{max_output_buffer_size}_losspen_{loss_penalty_type}_lambda_step_{lambda_train_step}_soft_ratio_{buffer_soft_limit}_wait_{lambda_wait}_lambda_lr_1e5_dt_time_{d_m_max_time}"
                                                        # launch training
                                                ...
                                                for model in ["sp", "opt"]:
                                                    ...
                                                    for test_load in test_loads:
                                                        python_command = generate_command(seed=seed,
                                                                                        ...
                                                                                        sim_duration=test_duration,
                                                                                        agent_type=f"{model}",
                                                                                        session_name=f"{model}_{traff_mat}_{test_duration}_final_",
                                                                                        traffic_matrix_index=traff_mat,
                                                                                        signaling_type="ideal", 
                                                                                        topology_name=topology_name,
                                                                                        experiment_name=experiment_name,
