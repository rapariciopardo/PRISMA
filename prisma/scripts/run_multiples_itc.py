...
dqn_models = ["sp", "opt"]
dqn_models = [""]
experiment_name = "ITC_NN_size_variations_experiment_rcpo_final_09_june_rcpo_episodic_offband_new"
...
d_t_send_all_destinations_list = [0, 1]
test_only = False
                                                    ...
                                                    for idx, dqn_model in enumerate(dqn_models):
                                                        for sync_step in sync_steps:
                                                            for d_m_max_time in d_t_max_times:
                                                                ...
                                                                session_name = f"sync_{sync_step}_mat_{traff_mat}_dqn_{dqn_model}_{signaling_type}_size_{nn_sizes[idx]}_{topology_name}_tr_{train_load}_sim_{train_duration}_{max_num_episodes}_lr_{lr}_bs_{bs}_outb_{max_output_buffer_size}_losspen_{loss_penalty_type}_lambda_step_{lambda_train_step}_ratio_{buffer_soft_limit}_wait_{lambda_wait}_lambda_lr_1e7_dt_time_{d_m_max_time}_ping_{pingPacketIntervalTime}_{exploration[0]}_{["one", "multi"][d_t_send_all_destinations]}"
                                                                ...
                                                                for test_load in test_loads:
                                                                    ...
                                                                    for model_version in ["final"]:
                                                                        python_command = generate_command(seed=seed,
                                                                                                        ...
                                                                                                        agent_type=f"dqn_buffer{dqn_model}",
                                                                                                        session_name=str(session_name)+f"_{test_duration}",
                                                                                                        traffic_matrix_index=traff_mat,
