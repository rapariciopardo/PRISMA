...
# dqn_models = [""]
experiment_name = "ITC_NN_size_variations_experiment_new"
starting_port = 700
...
inc = 0
command_name = "docker run --rm --gpus all -v /home/redha/PRISMA_copy/prisma/examples:/app/prisma/examples -w /app/prisma allicheredha/prismacopy:2.8.1"
        ...
        for idx, dqn_model in enumerate(dqn_models):
            if signaling_type == "NN":
                for sync_step in sync_steps:
                        ...
                        os.system(f'tsp {command_name} ./scripts/run_11_nodes.sh {sync_step} {seed} {traff_mat} dqn_buffer{dqn_model} {signaling_type} {ping_freq} {rb_size} {inc} abilene 0 {training_load} 1 {train_duration} "{session_name}" {experiment_name} {mv_avg_interval} {lr} {bs} {exploration[1]} {exploration[2]} {d_t_max_time} {nn_sizes[idx]}')
                        inc +=1
            else:
                sync_step = 1
                session_name = f"sync_{sync_step}_seed_{seed}_traff_mat_{traff_mat}_dqn_buffer{dqn_model}_{signaling_type}_ping_freq_{ping_freq}_rb_size_{rb_size}_abilene_train_load_{training_load}_simTime_{train_duration}_mv_avg_{mv_avg_interval}_lr_{lr}_bs_{bs}_explo_{exploration[0]}_{d_t_max_time}_{nn_sizes[idx]}"
