...
# static parameters
traff_mats = list(range(0, 11))
# traff_mats = [0,]
# traff_mats = [0,]
sync_steps = list(range(1, 5))
# sync_steps = [1, 2, 4]
# sync_steps = [2, ]
# sync_steps = [1,]
seed = 100
rb_size = 10000
signaling_types= ["NN", "digital_twin", "target", "ideal"]
# signaling_types = [""]
...
loss_pen_types = [ "fixed", "constrained", "None"]
...
inc = 0
command_root = "docker run --rm --gpus all -v /home/redha/PRISMA_copy/prisma/examples:/app/prisma/examples -v /mnt/journal_paper_results/abilene_overlay/saved_models:/mnt/journal_paper_results/abilene_overlay/saved_models -w /app/prisma allicheredha/prismacopy_episodes:offband"
                                                    ...
                                                    for idx, dqn_model in enumerate(dqn_models):
                                                        for sync_step in sync_steps:
                                                            for d_m_max_time in d_t_max_times:
                                                                if d_m_max_time != 5 and signaling_type == "NN":
                                                                    continue
                                                                if sync_step != 3 and (signaling_type == "digital_twin" or signaling_type == "target"):
                                                                    continue
                                                                ...
                                                                # saved_models_path = f"/mnt/journal_paper_results/abilene_overlay/saved_models/{session_name}"
                                                                ...
                                                                for test_load in test_loads:
                                                                    # for model_version in ["final"] + [f"episode_{i}_step_{i}" for i in range(1, 20, 1)]:
