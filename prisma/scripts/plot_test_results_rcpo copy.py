...
if topology_name == "abilene":
    ...
    for traff_mat in traff_mats:
        for metric_name in metric_names[f"test_results"]:
            session_name = f"sync_1_mat_{traff_mat}_dqn__digital_twin_size_{nn_size}_tr_{0.9}_sim_20_20_lr_1e-05_bs_512_outb_16260_losspen_constrained_lambda_step_-1_ratio_{0}_wait_{0}_lambda_lr_1e6_dt_time_3_ping_{0.1}_vary_one_explo_first_loss_{loss_flag}_reset_{0}_use_loss_{use_loss_flag}_threshold_{0.0}"
            ### add the data to the results
            pth = f"/mnt/backup_examples_new/overlay_full_mesh_10n_geant/results/geant_results_with_threshold/{session_name}/test_results/final"
            try:
                exp_df = convert_tb_data(pth)
            except:
                print(pth)
...
if topology_name == "abilene":
    ...
    for traff_mat in traff_mats:
            ...
            for threshold in [0.0, 0.25, 0.5, 0.75]:
                ...
                for metric_name in ["signalling ratio", "nb_new_pkts"]:
                    ...
                    if len(steps) > 1:
                        ...
                        data_to_store = np.array(data.value).reshape(-1, 1)
                    key = f"{signaling_type}_1_{loss_penalty_type}_0.9_0.1_{snapshot}_stats_{metric_name}_{threshold}"
                    if key not in all_exps_results.keys():
                        all_exps_results[key] = {"loads": steps.tolist(),
                                                "traff_mat": [traff_mat,],
                                                "data": data_to_store}
